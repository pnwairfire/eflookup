...
idcols = ['region','fuelcat','fuelsubcat','phase']
...
c2ef['covertype'] = c2ef.index
c2ef.sort_values('covertype', inplace=True)
c2ef.to_csv('covertype2efgroupname.csv', index=False, columns=['covertype','efgroupname'])

import covertype2efgroup
c2ef = pd.DataFrame.from_dict(covertype2efgroup.COVERTYPE_2_EF_GROUP, orient='index')
c2ef['covertype'] = c2ef.index
c2ef = pd.melt(c2ef, id_vars='covertype', var_name='phase', value_name='efgroup')
c2ef.sort_values(['covertype','phase'], inplace=True)
...
idcols = ['regionname','efgroupname','phase','species']
...
f2ct.columns = ['covertype',]
f2ct['fccs_id'] = f2ct.index
f2ct.sort_values('fccs_id', inplace=True)
f2ct.to_csv('fccs2covertype.csv', index=False, columns=['fccs_id','covertype'])
